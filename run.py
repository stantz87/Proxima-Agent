...
def init_db():
    ...
    """)
    # Add pdf_filename column if upgrading
    try:
    ...
    if cur.fetchone()[0]==0:
        ...
        for addr,notes in [
            ...
            con.execute("INSERT INTO deals (id,address,description,price,annual_rent,monthly,net_yield,lease_term,break_clause,term_income,status,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (str(uuid.uuid4()),addr,"3 Bed Semi-Detached · Freehold · No Chain · Immediate Completion",
                "£114,000","£11,400 p.a.","£950","10.0%","10yr FRI","None","£130,667","available",notes,now,now))
    con.commit()
    con.close()
...
def reserve():
    ...
    deal_id=data.get("deal_id"); name=data.get("name","").strip(); email=data.get("email","").strip()
    if not deal_id or not name or not email: return jsonify({"ok":False,"error":"Name and email required"})
    con=db()
    deal=con.execute("SELECT status FROM deals WHERE id=?",(deal_id,)).fetchone()
    if not deal or deal["status"]!="available":
        con.close(); return jsonify({"ok":False,"error":"Unit not available"})
    now=datetime.now().isoformat()
    con.execute("UPDATE deals SET status='reserved',updated_at=? WHERE id=?",(now,deal_id))
    con.execute("INSERT INTO reservations (id,deal_id,name,email,phone,notes,created_at) VALUES (?,?,?,?,?,?,?)",(str(uuid.uuid4()),deal_id,name,email,data.get("phone",""),data.get("notes",""),now))
    con.execute("INSERT INTO updates (id,deal_id,message,created_at) VALUES (?,?,?,?)",(str(uuid.uuid4()),deal_id,f"Reserved by {name} ({email})",now))
...
def add_deal():
    ...
    now=datetime.now().isoformat()
    con=db()
    con.execute("INSERT INTO deals (id,address,description,price,annual_rent,monthly,net_yield,lease_term,break_clause,term_income,status,notes,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",(str(uuid.uuid4()),f["address"],f.get("description",""),f.get("price",""),f.get("annual_rent",""),f.get("monthly",""),f.get("net_yield",""),f.get("lease_term",""),f.get("break_clause","None"),f.get("term_income",""),f.get("status","available"),f.get("notes",""),now,now))
    con.commit(); con.close()
...
def upload_photo():
    ...
    if not f or not f.filename or not allowed_img(f.filename): return redirect("/admin?err=Invalid+image+file")
    con=db()
...
def upload_pdf():
    ...
    if not f or not f.filename or not allowed_pdf(f.filename): return redirect("/admin?err=Invalid+PDF+file")
    filename=f"sheet_{did[:8]}.pdf"
    f.save(os.path.join(UPLOAD_FOLDER,filename))
    con=db()
    con.execute("UPDATE deals SET pdf_filename=? WHERE id=?",(filename,did))
    con.commit(); con.close()
    return redirect("/admin?msg=Investment+sheet+uploaded")
...
ADMIN_HTML = """<!DOCTYPE html><html><head><meta charset=UTF-8><title>Proxima Capital — Admin</title>
<style>
...
.ok{background:#e6fbf2;color:#059255;border:1px solid #b3f0d9}
.er{background:#fce8e8;color:#b93333;border:1px solid #f5b8b8}
...
.ch{background:#085454;color:#fff;padding:13px 18px;display:flex;align-items:center;justify-content:space-between}
.ca{font-size:14px;font-weight:800}
.cb{padding:18px}
.g3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;margin-bottom:16px}
.il label{font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#9aabab;margin-bottom:3px;display:block}
.vl{font-size:13px;font-weight:700;font-family:monospace}
...
.add-box{width:110px;height:80px;background:#f0f4f4;border-radius:7px;border:2px dashed #cdd;display:flex;align-items:center;justify-content:center;font-size:11px;color:#9aabab;cursor:pointer;text-align:center;flex-direction:column;gap:4px}
.pdf-box{background:#f0f4f4;border-radius:8px;padding:12px 14px;margin-bottom:14px;display:flex;align-items:center;justify-content:space-between;gap:12px}
.pdf-name{font-size:12px;font-weight:700;color:#085454;display:flex;align-items:center;gap:6px}
...
.res-item{background:#fff3e0;border:1px solid #ffd888;border-radius:7px;padding:9px 12px;margin-bottom:7px;font-size:12px}
...
.sel{padding:7px 10px;border:1px solid #cdd;border-radius:6px;font-size:12px;background:#fff}
...
.btn-red{background:#fce8e8;color:#b93333;font-size:10px;font-weight:800;padding:6px 12px;border-radius:6px;border:1px solid #f5b8b8;cursor:pointer}
.btn-green{background:#01C977;color:#024d30;font-size:10px;font-weight:800;letter-spacing:1px;text-transform:uppercase;padding:8px 14px;border-radius:6px;border:none;cursor:pointer}
.sa{color:#059255;background:#e6fbf2;border:1px solid #b3f0d9;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:800;text-transform:uppercase}
.sr{color:#c07a00;background:#fff3e0;border:1px solid #ffd888;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:800;text-transform:uppercase}
.so{color:#3a3d8f;background:#eaecf4;border:1px solid #c0c4e8;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:800;text-transform:uppercase}
.fb{background:#fff;border-radius:11px;border:1px solid #dde2e2;padding:22px}
.fg{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px}
.fg label{font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#535353;display:block;margin-bottom:4px}
.fg input,.fg select,.fg textarea{width:100%;padding:8px 10px;border:1px solid #cdd8d8;border-radius:7px;font-size:13px;background:#fff;outline:none}
textarea{resize:vertical;min-height:60px}
</style></head><body>
<div class=hdr>
  <div><div class=logo>Proxima <span>Capital</span></div><div class=hdr-sub>Investments — Admin Panel</div></div>
  <div class=nav><a href=/portal target=_blank>&#8599; View Portal</a><a href=/admin/logout>Sign Out</a></div>
</div>
<div class=body>
...
<div class=card>
  <div class=ch>
    ...
    <div style="display:flex;gap:10px;align-items:center">
      {% if d.status=='available' %}<span class=sa>Available</span>
      {% elif d.status=='reserved' %}<span class=sr>Reserved</span>
      {% else %}<span class=so>{{ d.status.title() }}</span>{% endif %}
  ...
  <div class=cb>
    <div class=g3>
      ...
      <div class=il><label>Net Yield</label><div class=vl style="color:#01a060">{{ d.net_yield }}</div></div>
    ...
    <div style="font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;color:#9aabab;margin-bottom:8px">Investment Sheet (PDF)</div>
    {% if d.pdf_filename %}
    <div class=pdf-box>
      <div class=pdf-name>&#128196; {{ d.pdf_filename }}</div>
      <div style="display:flex;gap:8px">
        <a href="/uploads/{{ d.pdf_filename }}" target="_blank" class=btn>View</a>
        <form method=POST action=/admin/delete-pdf style=margin:0>
          <input type=hidden name=deal_id value="{{ d.id }}">
    ...
    <div class=photo-row>
      ...
      <div style=position:relative>
        ...
        <form method=POST action=/admin/delete-photo style="position:absolute;top:3px;right:3px;margin:0">
          ...
          <button style="background:#b93333;color:#fff;border:none;border-radius:50%;width:20px;height:20px;font-size:13px;cursor:pointer">&times;</button>
        </form>
      </div>
      {% endfor %}
...
<title>Proxima Capital Investments — Property Portal</title>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{background:#f3f4f4;font-family:Inter,sans-serif;color:#222}
.hdr{background:#085454;border-bottom:3px solid #01C977;padding:18px 32px;display:flex;align-items:center;justify-content:space-between}
.logo{font-size:22px;font-weight:800;color:#fff;letter-spacing:-0.5px}.logo span{color:#01C977}
.hdr-sub{font-size:11px;letter-spacing:2px;text-transform:uppercase;color:rgba(255,255,255,0.45);margin-top:3px}
.agent-pill{background:rgba(1,201,119,0.12);border:1px solid rgba(1,201,119,0.3);color:#01C977;font-size:10px;font-weight:700;letter-spacing:1px;text-transform:uppercase;padding:6px 14px;border-radius:20px}
.stats{background:#085454;padding:0 32px 18px;display:grid;grid-template-columns:repeat(4,1fr);gap:14px}
.stat{background:rgba(255,255,255,0.07);border:1px solid rgba(255,255,255,0.09);border-radius:10px;padding:14px 16px}
.sl{font-size:9px;letter-spacing:1.5px;text-transform:uppercase;color:rgba(255,255,255,0.45);margin-bottom:6px}
.sv{font-size:22px;font-weight:800;color:#fff;font-family:monospace}.sv.g{color:#01C977}
.ss{font-size:10px;color:rgba(255,255,255,0.35);margin-top:3px}
.body{padding:28px 32px;max-width:980px}
.sec{font-size:11px;font-weight:800;letter-spacing:2px;text-transform:uppercase;color:#085454;margin-bottom:16px}
.card{background:#fff;border-radius:14px;border:1px solid #dde2e2;overflow:hidden;margin-bottom:20px;box-shadow:0 2px 8px rgba(8,84,84,0.05)}
.photos{display:grid;gap:3px}
.photos img{width:100%;height:200px;object-fit:cover;display:block}
.no-photo{height:160px;background:#e8f0f0;display:flex;align-items:center;justify-content:center;font-size:13px;color:#9aabab;letter-spacing:1px;text-transform:uppercase}
.cb{padding:22px 24px}
.addr{font-size:16px;font-weight:800;color:#085454;margin-bottom:3px}
.desc{font-size:12px;color:#535353;margin-bottom:13px}
.pills{display:flex;flex-wrap:wrap;gap:6px;margin-bottom:14px}
.p{font-size:9px;font-weight:800;letter-spacing:0.8px;text-transform:uppercase;padding:4px 9px;border-radius:4px}
...
.notes{font-size:12px;color:#535353;margin-bottom:14px;line-height:1.7;padding:12px 14px;background:#f7f9f9;border-radius:8px;border-left:3px solid #01C977}
.metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:16px}
.ml{font-size:9px;letter-spacing:1px;text-transform:uppercase;color:#9aabab;margin-bottom:3px}
.mv{font-size:14px;font-weight:800;font-family:monospace;color:#222}.mv.g{color:#01a060}
.bottom-row{display:flex;align-items:center;justify-content:space-between;padding-top:16px;border-top:1px solid #eff0f0;flex-wrap:wrap;gap:12px}
.yld{background:#085454;color:#01C977;font-size:24px;font-weight:800;padding:12px 18px;border-radius:10px;font-family:monospace;text-align:center;min-width:120px}
.ys{font-size:9px;color:rgba(255,255,255,0.4);letter-spacing:1px;text-transform:uppercase;margin-top:3px}
.sc{font-size:9px;font-weight:800;letter-spacing:1px;text-transform:uppercase;padding:5px 12px;border-radius:20px}
...
.btn-res{background:#01C977;color:#024d30;font-size:11px;font-weight:800;letter-spacing:0.8px;text-transform:uppercase;padding:10px 20px;border-radius:8px;border:none;cursor:pointer}
.btn-res:hover{opacity:0.85}.btn-res:disabled{opacity:0.35;cursor:not-allowed}
.btn-pdf{background:#085454;color:#fff;font-size:11px;font-weight:800;letter-spacing:0.8px;text-transform:uppercase;padding:10px 18px;border-radius:8px;border:none;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center;gap:6px}
.btn-pdf:hover{opacity:0.88}
.upds{margin-top:16px;padding-top:16px;border-top:1px solid #eff0f0}
.ut{font-size:9px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;color:#9aabab;margin-bottom:10px}
.ui{background:#f7f9f9;border-radius:7px;padding:9px 13px;margin-bottom:7px;font-size:12px;color:#444;display:flex;justify-content:space-between;gap:12px;border-left:2px solid #01C977}
.ud{font-size:10px;color:#9aabab;white-space:nowrap}
.foot{background:#f7f9f9;border-top:1px solid #eff0f0;padding:10px 24px;font-size:10px;color:#9aabab;display:flex;justify-content:space-between}
...
.modal{background:#fff;border-radius:14px;padding:28px;max-width:440px;width:100%;border-top:4px solid #01C977}
.mt{font-size:16px;font-weight:800;color:#085454;margin-bottom:3px}
.ms{font-size:12px;color:#535353;margin-bottom:18px}
.ml2{font-size:9px;font-weight:800;letter-spacing:1.2px;text-transform:uppercase;color:#535353;margin-bottom:4px;display:block;margin-top:12px}
.mi{width:100%;padding:10px 12px;border:1px solid #cdd;border-radius:8px;font-size:13px;color:#222;outline:none}
.mi:focus{border-color:#085454}
.macts{display:flex;gap:10px;margin-top:20px}
.msub{flex:1;background:#085454;color:#fff;font-size:11px;font-weight:800;letter-spacing:1px;text-transform:uppercase;padding:11px;border-radius:8px;border:none;cursor:pointer}
.mcan{background:#fff;color:#535353;font-size:11px;padding:11px 16px;border-radius:8px;border:1px solid #cdd;cursor:pointer}
.toast{position:fixed;bottom:22px;right:22px;background:#085454;color:#fff;padding:12px 20px;border-radius:10px;font-size:13px;font-weight:700;border-left:4px solid #01C977;opacity:0;transform:translateY(8px);transition:all 0.3s;pointer-events:none;z-index:999}
.toast.show{opacity:1;transform:translateY(0)}
</style></head><body>
<div class=hdr>
  <div>
    <div class=logo>Proxima <span>Capital</span></div>
    <div class=hdr-sub>Investments &nbsp;&middot;&nbsp; Property Portal</div>
  </div>
  <div class=agent-pill>&#10003;&nbsp; Authorised Agent Access</div>
</div>
<div class=stats>
  <div class=stat><div class=sl>Live Deals</div><div class=sv>{{ deals|length }}</div><div class=ss>Hartlepool</div></div>
  ...
  <div class=stat><div class=sl>Lease Term</div><div class=sv style="font-size:15px;padding-top:5px">10yr FRI</div><div class=ss>No break clause</div></div>
</div>
<div class=body>
<div class=sec>Property Investments</div>
...
<div class=card>
  ...
  <div class=cb>
    ...
    <div class=pills>
      <span class="p ps">FRI Lease</span>
      <span class="p pg">Exempt Accommodation</span>
      <span class="p ps">{{ d.lease_term }}</span>
      <span class="p pg">Day 1 Income</span>
      <span class="p pr">No Break Clause</span>
      <span class="p pg">HB Backed</span>
    </div>
    ...
    <div class=metrics>
      ...
      <div><div class=ml>10yr Projected Income</div><div class=mv>{{ d.term_income }}</div></div>
    ...
    <div class=bottom-row>
      ...
      <div class=actions>
        {% if d.pdf_filename %}
        <a href="/uploads/{{ d.pdf_filename }}" target="_blank" class=btn-pdf>&#128196; Investment Sheet</a>
        {% endif %}
        <button class=btn-res {% if d.status!='available' %}disabled{% endif %}
          onclick="openModal('{{ d.id }}','{{ d.address }}','{{ d.price }} &middot; {{ d.net_yield }} yield &middot; {{ d.annual_rent }}')">
          &#128274; Reserve This Unit
        </button>
      </div>
    </div>
    {% if updates %}
    <div class=upds>
      <div class=ut>Progress Updates</div>
...
{% endfor %}
<p style="font-size:11px;color:#9aabab;margin-top:24px;line-height:1.8">For sophisticated and high-net-worth investors only. Capital is at risk. This portal does not constitute financial advice.<br>invest@proxcap.co &nbsp;&middot;&nbsp; +44 20 8368 9966 &nbsp;&middot;&nbsp; proxcap.co</p>
</div>
<div class=modal-bg id=modal>
  <div class=modal>
    <div class=mt id=m-title>Reserve Unit</div>
    <div class=ms id=m-sub></div>
    <span class=ml2>Investor Name</span><input class=mi id=m-name placeholder="Full name">
    <span class=ml2>Email Address</span><input class=mi id=m-email placeholder="investor@email.com">
    ...
    <div class=macts>
      ...
      <button class=msub onclick="submitRes()">Confirm Reservation</button>
    </div>
  </div>
</div>
...
if __name__ == "__main__":
    init_db()
    print("\n" + "="*55)
    print("  PROXIMA CAPITAL — PROPERTY PORTAL")
    print("="*55)
